...
void CCOllisionMgr::Collision_Object_Wall(list<CObj*>* pObjs, vector<CObj*>* pWalls)
{
	...
	for (auto* pObj : *pObjs)
	{
		...
		POINT pt = { pObj->Get_Info()->fX, pObj->Get_Info()->fY };
		if (!PtInRect(&rc, pt))
			continue;
		RECT temp = {};
...
bool CCOllisionMgr::Collision_Shop_Bullet(RECT * pInfo, list<CObj*>* pBullets, list<CObj*>* pRazers)
{
	for (auto& pBullet : *pBullets)
	{
		POINT pt = { pBullet->Get_Info()->fX, pBullet->Get_Info()->fY };
	...
	{
		POINT pt = { pRazer->Get_Info()->fX, pRazer->Get_Info()->fY };
		if (PtInRect(pInfo, pt))
		{
			return true;
		}
...
void CCOllisionMgr::Collision_Shop_Buy(CObj * pShop, CObj * pPlayer)
{
	...
	POINT pt = { pPlayer->Get_Info()->fX, pPlayer->Get_Info()->fY };
	if (!PtInRect(&rt, pt))
		return;
...
{
	POINT pt = { pMouse->Get_Info()->fX, pMouse->Get_Info()->fY };

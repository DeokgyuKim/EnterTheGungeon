	...
	for (int i = iCullY; i < iCullEndY + 2; ++i)
	{
		for (int j = iCullX; j < iCullEndX + 2; ++j)
		{
			...
			if (bGuideLine)
			{
				...
				TextOut(hDC, m_vecTile[iIndex]->Get_Info()->fX + CScrollMgr::Get_MouseScroll().fX - 20
					, m_vecTile[iIndex]->Get_Info()->fY + CScrollMgr::Get_MouseScroll().fY, szPos, lstrlen(szPos));
			}
#endif
		}
	}

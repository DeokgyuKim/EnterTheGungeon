...
void CShop::Render(HDC hDC)
{
	...
	GdiTransparentBlt(hDC, m_tRect.left + tPos.fX, m_tRect.top + tPos.fY, m_tInfo.iCX, m_tInfo.iCY,
		hMemDC, m_tInfo.iCX * m_tFrame.iFrameStart, 0, m_tInfo.iCX, m_tInfo.iCY, RGB(255, 255, 255));

#ifdef _DEBUG
	for (int i = 0; i < 4; ++i)
	{
		Rectangle(hDC, m_tCollisionRect[i].left + tPos.fX, m_tCollisionRect[i].top + tPos.fY,
			m_tCollisionRect[i].right + tPos.fX, m_tCollisionRect[i].bottom + tPos.fY);
	}
#endif
